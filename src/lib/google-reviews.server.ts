...
function curatedFallback(): GoogleReviewsPayload {
  ...
  const profileUrl =
    ...
    `https://www.google.com/maps/place/?q=place_id:${preset.googlePlaceId ?? ""}`;
  const rating = preset.id === "proklim" ? 5 : 4.9;
  const reviewCount = preset.id === "proklim" ? 24 : 48;

  return buildCuratedPayload(preset.reviews, profileUrl, rating, reviewCount);
}
