...
function StepIcons({ step, icon: Icon }: { step: number; icon: LucideIcon }) {
  return (
    <div className="flex items-center gap-2">
      <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gradient-accent text-sm font-bold text-primary-foreground shadow-glow">
        {step}
      </span>
      <div className="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-white/10 text-brand-cyan">
...
function DesktopStepCard({ step, icon, title, desc }: Step) {
  const Icon = icon;
  return (
    <li className="rounded-2xl border border-white/10 bg-white/5 p-6 text-left backdrop-blur-md">
...
export function HowItWorks() {
  return (
    <section
      id="jak-dzialamy"
      className="scroll-mt-24 bg-brand-deep px-4 py-6 text-primary-foreground md:py-14"
    >
      <div className="mx-auto max-w-6xl text-center">
        <p className="text-xs font-semibold uppercase tracking-widest text-brand-cyan">Proces</p>
        <h2 className="mt-1.5 text-xl font-bold tracking-tight text-white md:mt-2 md:text-3xl">
          Jak to działa?
        </h2>
        ...
        <div className="mx-auto mt-5 max-w-md rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur-md md:hidden">
          <TimelineSteps />
        </div>

        <ol className="mt-8 hidden gap-6 md:grid md:grid-cols-3 md:text-left">
          {STEPS.map((item) => (
            <DesktopStepCard key={item.step} {...item} />
          ))}
        </ol>
      </div>
    </section>
